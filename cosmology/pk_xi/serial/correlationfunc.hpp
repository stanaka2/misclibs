...
private:
  ...
  template <typename T>
  std::vector<T> xismu_to_xir_jk(T &, T &, T &);
  template <typename T>
  T xispsp_to_wpr(const T &, const T &) const;
  template <typename T>
  std::vector<T> xispsp_to_wpr_jk(T &, T &, T &);
...
#pragma omp parallel for
  for(int i = 0; i < nn; i++) {
    ...
    xi_sd[i] = sqrt(variance[i]);
    xi_se[i] = xi_sd[i] / sqrt((double)njk);
  }
...
template <typename T>
std::vector<T> correlation::xismu_to_xir_jk(T &ave1d, T &sd1d, T &se1d)
{
  ...
  for(int ir = 0; ir < nr; ++ir) {
    ...
    sd1d[ir] = std::sqrt(var);
    se1d[ir] = sd1d[ir] / std::sqrt((double)njk);
  }
...
template <typename T>
std::vector<T> correlation::xispsp_to_wpr_jk(T &ave1d, T &sd1d, T &se1d)
{
...
void correlation::output_xi1D_smu(std::string filename)
{
  ...
  if(jk_level <= 1) {
    ...
    std::vector<double> ave, sd, se;
    auto xir_blocks = xismu_to_xir_jk(ave, sd, se);
    fout << "# r[Mpc/h] xi_ave SD SE block1 block2 block3 ..." << std::endl;
    for(int ir = 0; ir < nr; ++ir) {
      const double r = rcen[ir] * lbox;
      fout << std::scientific << std::setprecision(10) << r << " " << ave[ir] << " " << sd[ir] << " " << se[ir];
...
void correlation::output_xi1D_spsp(std::string filename)
{
  ...
  if(jk_level <= 1) {
    ...
    std::vector<double> ave, sd, se;
    auto wp_blocks = xispsp_to_wpr_jk(ave, sd, se);
    fout << "# r_perp[Mpc/h] wp_ave SD SE block1 block2 block3 ..." << std::endl;
    for(int ip = 0; ip < nsperp; ++ip) {
      const double rp_phys = sperpcen[ip] * lbox;
      fout << std::scientific << std::setprecision(10) << rp_phys << " " << ave[ip] << " " << sd[ip] << " " << se[ip];
